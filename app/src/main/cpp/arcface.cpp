...
vector<float>Arcface::new_getFeature(ncnn::Mat img, FaceInfo info){
    ncnn::Mat det = preprocess(img, info);
    return getFeature(det);
}
...
float new_calcSimilar(ncnn::Mat img1, int height1, int width1, FaceInfo info1,
    ...
    Arcface arc(path);
    vector<float> feature1 = arc.new_getFeature(img1, info1);
    vector<float> feature2 = arc.new_getFeature(img2, info2);
