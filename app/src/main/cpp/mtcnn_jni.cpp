...
Java_com_mtcnn_1insightface_MTCNN_FaceDetectionModelInit(JNIEnv *env, jobject instance,
...
Java_com_mtcnn_1insightface_MTCNN_FaceDetect(JNIEnv *env, jobject instance, jbyteArray imageDate_,
...
JNIEXPORT jintArray JNICALL
Java_com_mtcnn_1insightface_MTCNN_MaxFaceDetect(JNIEnv *env, jobject instance, jbyteArray imageDate_,
                                       jint imageWidth, jint imageHeight, jint imageChannel) {
    //  LOGD("JNI开始检测人脸");
    if(!detection_sdk_init_ok){
        LOGD("人脸检测MTCNN模型SDK未初始化，直接返回空");
    ...
    //  LOGD("内部人脸检测完成,开始导出数据");
    int *faceInfo = new int[out_size];
    faceInfo[0] = num_face; //数组第一个值存放检测到的人脸数量
    for(int i=0;i<num_face;i++){
        faceInfo[14*i+1] = finalBbox[i].x1; //某人脸框左上角的x坐标值
        faceInfo[14*i+2] = finalBbox[i].y1; //某人脸框左上角的y坐标值
        faceInfo[14*i+3] = finalBbox[i].x2; //某人脸框右下角的x坐标值
        faceInfo[14*i+4] = finalBbox[i].y2; //某人脸框右下角的y坐标值
        for (int j =0;j<10;j++){
            faceInfo[14*i+5+j]=static_cast<int>(finalBbox[i].ppoint[j]);
            //存储的10个值从左到右顺序存储着五个关键点的x坐标和y坐标
        }
    }
...
JNIEXPORT jboolean JNICALL
Java_com_mtcnn_1insightface_MTCNN_FaceDetectionModelUnInit(JNIEnv *env, jobject instance) {
    if(!detection_sdk_init_ok){
...
JNIEXPORT jboolean JNICALL
Java_com_mtcnn_1insightface_MTCNN_SetMinFaceSize(JNIEnv *env, jobject instance, jint minSize) {
    if(!detection_sdk_init_ok){
...
JNIEXPORT jboolean JNICALL
Java_com_mtcnn_1insightface_MTCNN_SetThreadsNumber(JNIEnv *env, jobject instance, jint threadsNumber) {
    if(!detection_sdk_init_ok){
        LOGD("人脸检测MTCNN模型SDK未初始化，直接返回");
        return false;
    }
...
JNIEXPORT jboolean JNICALL
Java_com_mtcnn_1insightface_MTCNN_SetTimeCount(JNIEnv *env, jobject instance, jint timeCount) {
